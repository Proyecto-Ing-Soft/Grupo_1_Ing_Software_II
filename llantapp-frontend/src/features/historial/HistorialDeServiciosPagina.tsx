...
export default function HistorialDeServiciosPagina() {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      console.log('[HIST] effect start', { id, tieneToken: !!usuario?.token, tokenPreview: usuario?.token?.slice?.(0, 12) });
      if (!id || !usuario?.token) {
      ...
      try {
        ...
        console.log('[HIST] fetching /historial/vehiculo/:id', { vehiculoId: Number(id) });
        ...
        try {
          if (err instanceof Response) {
            ...
            console.error('[HIST] HTTP error', { status: err.status, msg, rawText: txt });
          } else if (err?.message) {
            msg = err.message;
  ...
  console.log('[HIST] render', {
    cargando, tieneData: !!data, tieneError: !!error,
            ...
            {data.proximosServicios.map((s: ProximoServicio) => (
              <div key={s.id} className="timeline-item timeline-item--proximo">
                ...
                <div className="timeline-item__content">
                  ...
                  <p className="timeline-item__description">{s.comentario || 'Sin comentarios.'}</p>
