...
export default function CatalogoServiciosPagina() {
  ...
  // Mock inicial (reemplazar luego por apiServicios.listar())
  const [items, setItems] = useState<Servicio[]>([
    { id: 1, nombre: "Mantenimiento preventivo", descripcion: "Cambio de aceite, filtros y revisión general.", estado: "ACTIVO" },
    { id: 2, nombre: "Alineación y balanceo", descripcion: "Ajuste de ángulos y balance de ruedas.", estado: "INACTIVO" },
  ]);
  ...
  const filtrados = useMemo(() => {
    ...
    return items.filter(x =>
      x.nombre.toLowerCase().includes(s) || x.descripcion.toLowerCase().includes(s)
    );
  }, [q, items]);

  // formulario
  const [form, setForm] = useState<Partial<Servicio>>({ estado: "ACTIVO" });
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver((entries) => {
      for (const e of entries) e.isIntersecting ? e.target.classList.add("animate-in") : e.target.classList.remove("animate-in");
    }, { threshold: 0.12 });
  ...
  const guardar = () => {
    ...
    // unicidad (frontend)
    const duplicado = items.some(s => s.nombre.toLowerCase() === nombre.toLowerCase() && s.id !== editId);
    if (duplicado) return setErr("Ya existe un servicio con ese nombre.");

    if (editId) {
      setItems(prev => prev.map(s => s.id === editId ? { ...s, nombre, descripcion, estado } : s));
      setOk("Servicio actualizado.");
    } else {
      const nuevo: Servicio = { id: ++_auto, nombre, descripcion, estado };
    ...
    <main className="svc">
      <header className="svc__header svc__stack-lg">
        <div className="svc__titleWrap reveal" data-reveal="1">
          ...
          <p className="svc__sub">Crea, edita y activa/inactiva ítems del catálogo.</p>
        </div>
      ...
      <section className="svc__content svc__stack-xl">
          ...
          <section className="svc__panel reveal" data-reveal="3" aria-live="polite">
            ...
            <div className="svc__form">
              ...
              <div className="svc__formActions">
                <button type="button" className="mc-btn mc-btn--gradient" onClick={guardar}>
                  💾 Guardar
                </button>
              </div>
        ...
        <div className="svc__grid reveal" data-reveal="4" role="list">
          {filtrados.map((s, idx) => (
            <article key={s.id} role="listitem" className={`svc__card is-toned ${s.estado === "ACTIVO" ? "svc--act" : "svc--ina"}`} data-reveal={String((idx % 5) + 1)}>
              <header className="svc__cardHeader">
                ...
                <div className="svc__cardActions">
                  ...
                  <button className="btnGhost" onClick={() => borrar(s.id)}>🗑️ Eliminar</button>
                </div>
              </header>
