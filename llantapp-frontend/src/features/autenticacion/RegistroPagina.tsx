...
function SolicitudTaller({ fondo }: { fondo: string }) {
  ...
  const validar = () => {
    ...
    if (!data.ruc || !soloDigitos(data.ruc) || data.ruc.length !== 11)
      e.ruc = "RUC inválido (11 dígitos).";
    if (!data.contactoNombre.trim())
      e.contactoNombre = "Ingresa el nombre de contacto.";
    if (!validarEmail(data.contactoCorreo))
      e.contactoCorreo = "Correo inválido.";
    if (!data.telefono.trim())
      e.telefono = "Ingresa un teléfono de contacto (Perú).";
    if (!data.departamento.trim())
      e.departamento = "Selecciona el departamento.";
    if (!data.ciudad.trim()) e.ciudad = "Ingresa la ciudad.";
    if (!data.direccion.trim())
      e.direccion = "Ingresa la dirección.";
    if (data.numSedes && !soloDigitos(data.numSedes))
      e.numSedes = "Solo números.";
    return e;
  };
  ...
  const enviar = async (e: React.FormEvent) => {
    ...
    if (Object.keys(ve).length) {
      setErr(ve);
      setFailMsg("Revisa los campos marcados en rojo.");
      return;
    }

    setEnviando(true);
    try {
      // Construimos una dirección "enriquecida" con ciudad/depto y comentario
      const direccionCompuesta = [
        data.direccion,
        data.ciudad,
        data.departamento,
        data.numSedes ? `Sedes: ${data.numSedes}` : "",
        data.mensaje ? `Nota: ${data.mensaje}` : "",
      ]
        .filter(Boolean)
        .join(" | ");

      const resp = await apiSolicitudesTaller.crearPublica({
        razonSocial: data.razonSocial.trim(),
        ruc: data.ruc.trim(),
        direccion: direccionCompuesta || undefined,
        telefono: data.telefono.trim() || undefined,
        emailContacto: data.contactoCorreo.trim(),
        nombreContacto: data.contactoNombre.trim(),
        // usamos el mismo contacto como admin inicial
        adminNombre: data.contactoNombre.trim(),
        adminEmail: data.contactoCorreo.trim(),
      });
      ...
      console.error("❌ Error al enviar solicitud de taller", error);
      setFailMsg(
        error?.message
    ...
    >
      ...
      >
        <div className="auth-left">
          ...
          <p className="sub">
            El servicio está disponible <b>solo en Perú</b>. Déjanos tus
            datos; te contactaremos para configurar tu entorno.
          </p>
          ...
          <div
            ...
            aria-label="Precios y características de LlantApp para talleres"
          >
            <div className="pricing-title">
              LlantApp para talleres incluye:
            </div>
            <ul
              style={{
                margin: 0,
            ...
            >
              <li>
                Recepción de vehículos con checklist y fotos
              </li>
              <li>
                Estados claros (programado, en proceso, listo) con
                notificaciones
              </li>
              <li>
                Historial por vehículo con evidencias y firmas
              </li>
              <li>
                Catálogo de servicios y asignación por mecánico
              </li>
              <li>
                Calificaciones de clientes y reportes básicos
              </li>
            </ul>

            <div
              className="pricing-rows"
              style={{ marginTop: 12 }}
            >
              <div className="pricing-row">
                <div className="price-now">
                  Instalación (pago único): S/ {PRECIO_INSTALACION_HOY_S}*
                </div>
                <div className="price-old">
                  S/ {PRECIO_INSTALACION_LISTA_S}
                </div>
              ...
              <div className="pricing-row">
                <span aria-hidden="true">•</span>
                <div className="price-now">
                  Soporte mensual: S/ {PRECIO_SOPORTE_MENSUAL_HOY_S}
                  /mes
                </div>
                <div className="price-old">
                  S/ {PRECIO_SOPORTE_MENSUAL_LISTA_S}/mes
                </div>
              </div>
              <div className="pricing-row">
                <span aria-hidden="true">•</span>
                <div className="price-now">
                  Soporte anual: S/ {PRECIO_SOPORTE_ANUAL_HOY_S}/año
                </div>
                <div className="price-old">
                  S/ {PRECIO_SOPORTE_ANUAL_LISTA_S}/año
                </div>
                <div className="price-note">
                  Incluye actualizaciones y ayuda prioritaria
                </div>
              </div>
              <div className="pricing-row">
                <div className="price-note" id="letrapequena">
                  * Monto mínimo sujeto a cotización. Suele aumentar
                  bastante.
                </div>
              </div>
            </div>
          ...
          <div className="form">
            ...
            <div className="form-group">
              ...
              <div
                className={`input-wrap ${
                  err["razonSocial"] ? "has-error" : ""
                }`}
              >
                <span
                  className="iconbox fa-regular fa-building"
                  aria-hidden="true"
                />
                <input
                  id="razonSocial"
            ...
            <div className="form-group">
              ...
              >
                <span
                  className="iconbox fa-solid fa-id-card"
                  aria-hidden="true"
                />
                <input
                  id="ruc"
            ...
            {/* Contacto */}
            <div className="form-group">
              <label className="label" htmlFor="contactoNombre">
                Nombre de contacto
              </label>
              <div
                className={`input-wrap ${
                  err["contactoNombre"] ? "has-error" : ""
                }`}
              >
                <span
                  className="iconbox fa-regular fa-user"
            ...
            <div className="form-group">
              <label className="label" htmlFor="contactoCorreo">
                Correo de contacto
              </label>
              <div
                className={`input-wrap ${
                  err["contactoCorreo"] ? "has-error" : ""
                }`}
              >
                <span
                  className="iconbox fa-regular fa-envelope"
            ...
            <div className="form-group">
              ...
              <div
                className={`input-wrap ${
                  err["telefono"] ? "has-error" : ""
                }`}
              >
                <span
                  className="iconbox fa-solid fa-phone"
                  aria-hidden="true"
                />
                <input
                  id="telefono"
            ...
            <div className="form-group">
              ...
              <div
                className={`input-wrap ${
                  err["departamento"] ? "has-error" : ""
                }`}
              >
                <span
                  className="iconbox fa-solid fa-location-dot"
                  aria-hidden="true"
                />
                <select
                  id="departamento"
            ...
            <div className="form-group">
              ...
              <div
                className={`input-wrap ${
                  err["ciudad"] ? "has-error" : ""
                }`}
              >
                <span
                  className="iconbox fa-solid fa-city"
                  aria-hidden="true"
                />
                <input
                  id="ciudad"
            ...
            <div className="form-group">
              ...
              <div
                className={`input-wrap ${
                  err["direccion"] ? "has-error" : ""
                }`}
              >
                <span
                  className="iconbox fa-solid fa-road"
                  aria-hidden="true"
                />
                <input
                  id="direccion"
            ...
            <div className="form-group">
              <label className="label" htmlFor="numSedes">
                Número de sedes (opcional)
              </label>
              <div
                className={`input-wrap ${
                  err["numSedes"] ? "has-error" : ""
                }`}
              >
                <span
                  className="iconbox fa-regular fa-map"
                  aria-hidden="true"
                />
                <input
                  id="numSedes"
        ...
        <div className="full-row">
          <div className="form-group">
            ...
            <div className="input-wrap">
              <span
                className="iconbox fa-regular fa-comment-dots"
                aria-hidden="true"
              />
              <textarea
                id="mensaje"
          ...
          <p className="helper" style={{ marginTop: 10 }}>
            ...
            <a
              className="textlink"
              href="mailto:soporte.llantapp@gmail.com"
            >
              soporte.llantapp@gmail.com
            </a>{" "}
            <br />
            o llamar al <b>+51 915 060 423</b>.
          </p>
        </div>
