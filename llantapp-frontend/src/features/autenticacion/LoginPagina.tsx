...
// UI -> códigos de rol del back válidos para cada tipo de login.
// Todos los códigos vienen de BD (ej: OWNER, ADMIN_TALLER, MECANICO, CLIENTE).
const ROL_MAP: Record<RolUi, RolApi[]> = {
  cliente: ["CLIENTE"],
  // Desde /login?rol=taller deben ingresar admins de taller, mecánicos y owner.
  taller: ["ADMIN_TALLER", "OWNER", "MECANICO"],
};

// Back -> UI (para mensajes y sugerir por dónde entrar)
const rolApiToUi = (r: RolApi): RolUi =>
  r === "ADMIN_TALLER" || r === "OWNER" || r === "MECANICO"
    ? "taller"
    : "cliente";
...
export default function LoginPagina() {
  ...
  // Una vez que el login se completa y ya tenemos perfil en contexto,
  // validamos que el rol sea compatible con la ruta y redirigimos.
  useEffect(() => {
    if (!loginEnCurso) return;
    if (!sesion.accessToken || !sesion.perfil) return;

    const perfil = sesion.perfil;
    const rolesPermitidos = ROL_MAP[rolUi];

    if (!rolesPermitidos.includes(perfil.rol)) {
      const sugerido = rolApiToUi(perfil.rol);
      setFormErr(
        `Estás entrando por “${rolUi}”, pero tu cuenta es de tipo “${perfil.rol}”. Ingresa por /login/${sugerido}.`,
      );
      setLoginEnCurso(false);
      return;
    }

    const destinoSlug = perfil.tallerSlug || slugTaller;
    navigate(destinoSlug ? `/${destinoSlug}/inicio` : "/inicio");
    setLoginEnCurso(false);
  }, [loginEnCurso, sesion, rolUi, slugTaller, navigate]);
  ...
  const enviar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Cliente global: /login/cliente SIN slug -> login-cliente-global.
      // Taller (OWNER/ADMIN/MECANICO): viene desde /:slug/login/taller -> login con slug.
      const tipoLogin =
        rolUi === "cliente" && !slugTaller ? "clienteGlobal" : "taller";
  ...
  const titulo =
    rolUi === "taller" ? "Bienvenido Taller" : "Bienvenido Cliente";
  const baseSlugPath = slugTaller ? `/${slugTaller}` : "";
  const linkRegistroCliente = `${baseSlugPath}/registro/cliente`;
  const linkSolicitudTaller = `${baseSlugPath}/registro/taller`;
    ...
    <>
      ...
      >
        <section
          ...
          role="region"
          aria-label={`Formulario de inicio de sesión (${rolUi})`}
        >
          <div className="auth-left">
            ...
            <h1 className="brand reveal" data-reveal="2">
              {titulo}
            </h1>
            <p className="sub reveal" data-reveal="2">
            ...
            >
              <div className="form-group">
                ...
                {fieldErr["correo"] && (
                  <div id="err-correo" className="error-message">
                    {fieldErr["correo"]}
                  </div>
                )}
              </div>
              ...
              <div className="form-group">
                ...
                {fieldErr["clave"] && (
                  <div id="err-clave" className="error-message">
                    {fieldErr["clave"]}
                  </div>
                )}
              </div>
            ...
            {rolUi === "cliente" ? (
              <p className="helper reveal" data-reveal="4">
                ¿No tienes cuenta?{" "}
                <Link to={linkRegistroCliente} className="textlink">
                  Regístrate aquí
                </Link>
              </p>
            ) : (
              <p className="helper reveal" data-reveal="4">
                ...
                <Link to={linkSolicitudTaller} className="textlink">
          ...
          >
            ...
            <div className="auth-right-inner">
              <h2 className="hero-title reveal" data-reveal="1">
                {rolUi === "cliente"
                  ? "Tu historial y evidencias, en un solo lugar"
                  : "Transparencia técnica para tu taller"}
              </h2>
